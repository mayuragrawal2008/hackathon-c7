...
Two views: Landing -> Play. The "results screen" is an inline panel inside the play
view (sections toggled), because swapping separate top-level Columns crashes Gradio 6.
Standalone presentation layer over the same boxed LLM judge (Groq / Llama 3.3). The
graded submission remains the baseline HTML app on `main`.
...
def route_view(g):
    """Visibility-ONLY response (the pattern Gradio renders reliably).
    Outputs: play_area, result_area."""
    if g.get("view") == "result":
        return HIDE, SHOW
    return SHOW, HIDE
...
# ---------- handlers ----------
def enter_game():
    return HIDE, SHOW  # landing hidden, play view shown
...
def start_concept(concept_name, g, chat):
    ...
    return g, chat, stats_md(g), ""  # last "" clears the result banner


def send(msg, g, chat):
    """Content-only response (visibility handled by route_view via .then).
...
with gr.Blocks(title="Concept Check — Game") as demo:
    ...
    # -------- PLAY VIEW (single screen; result banner appears on top) --------
    with gr.Column(visible=False) as play_view:
        gr.HTML("<div id='cc-title'>🧩 Concept Check</div>")
        # result banner — always present, empty until a round ends (no visibility toggling)
    ...
    # -------- wiring (content-only updates — Gradio-safe, no in-game visibility toggles) --------
    enter_btn.click(enter_game, None, [landing, play_view])
    start_btn.click(start_concept, [concept_dd, game, chatbot], [game, chatbot, stats, result_box])
    send_btn.click(send, [box, game, chatbot], [game, chatbot, stats, box, result_box])
    box.submit(send, [box, game, chatbot], [game, chatbot, stats, box, result_box])
